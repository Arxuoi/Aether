...
local BG_TRANS   = 0.85   -- glass transparency
...
function Nexuralib:CreateWindow(config)
    ...
    -- ** PERBAIKAN 1: SESUAIKAN UKURAN DENGAN LAYAR **
    local screenSize = GetScreenSize()
    local maxWidth = screenSize.X * 0.85
    local maxHeight = screenSize.Y * 0.85
    winSize = Vector2.new(math.min(winSize.X, maxWidth), math.min(winSize.Y, maxHeight))
    
    -- Posisi otomatis di tengah, tapi bisa di-override winPos
    local defaultPosX = (screenSize.X - winSize.X) / 2
    local defaultPosY = (screenSize.Y - winSize.Y) / 2
    local containerPos = winPos and UDim2.new(0, winPos.X, 0, winPos.Y) or UDim2.new(0, defaultPosX, 0, defaultPosY)
